...
### Option A: Full repo AppHost + MCP Inspector

Use this flow when you want the AppHost to launch all local MCP server samples and provide a preconfigured MCP Inspector.
...
1. Start the AppHost from the repo root:
   ...
   The AppHost runs dedicated `functions-mcp-app-ui-*` and `mcp-weather-app-ui-*` JavaScript resources that restore, build, and watch the embedded Vite UIs before the matching Function apps start.

2. Open the `mcp-inspector` resource in the Aspire dashboard. The Inspector is preconfigured with:

   - `functions-mcp-tool` (default)
   - `functions-mcp-resources`
   - `functions-mcp-prompts`
   - `functions-mcp-app`
   - `mcp-weather-app`

3. Click **Connect**, then use the server dropdown to switch servers and inspect the tool, prompt, or resource samples.

   If you edit either UI while the AppHost is running, rebuild the matching Function resource so it picks up the refreshed `app/dist` bundle:
...
### Option B: Start only this Function app
...
### Option B: MCP Inspector via the AppHost
...
2. Open the `mcp-inspector` resource in the Aspire dashboard.
3. `functions-mcp-tool` is selected by default. Click **Connect**, then **List Tools** and **Run Tool**.
